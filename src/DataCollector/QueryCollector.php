...
class QueryCollector extends AbstractDataCollector implements AssetProvider
{
	...
	{
		return [
			'css' => Uri::root(true) . '/plugins/system/debug4/media/widgets/sqlqueries/widget.min.css',
			'js' => Uri::root(true) . '/plugins/system/debug4/media/widgets/sqlqueries/widget.min.js'
		];
	}
...
//					if (\Joomla\Database\DatabaseDriver::class === $class && false === strpos($file, 'DatabaseDriver.php'))
					if (\JDatabaseDriver::class === $class && false === strpos($file, 'DatabaseDriver.php'))
					{
