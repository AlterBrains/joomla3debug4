...
class InfoCollector extends AbstractDataCollector implements AssetProvider
{
	...
	{
		return [
			'js' => Uri::root(true) . '/plugins/system/debug4/media/widgets/info/widget.min.js',
			'css' => Uri::root(true) . '/plugins/system/debug4/media/widgets/info/widget.min.css',
		];
	}
...
//			->getMVCFactory()->createModel('Sysinfo', 'Administrator');
		\JModelLegacy::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_admin/models', 'AdminModel');
		$model = \JModelLegacy::getInstance( 'SysInfo' , 'AdminModel' );
