...
class LanguageErrorsCollector extends AbstractDataCollector implements AssetProvider
{
	...
	{
		return [
			'js' => Uri::root(true) . '/plugins/system/debug4/media/widgets/languageErrors/widget.min.js',
			'css' => Uri::root(true) . '/plugins/system/debug4/media/widgets/languageErrors/widget.min.css',
		];
	}
	...
	private function getData(): array
	{
		...
		if (\count($errorFiles))
		{
			...
			{
				// Joomla3
				list(, $lines) = explode(' : error(s) in line(s) ', $lines);
				$lines = explode(', ', $lines);
